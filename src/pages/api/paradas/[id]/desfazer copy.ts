// src/pages/api/paradas/desfazer.ts
...
export default async function handler(
  ...
  // id pode vir por query ou body (compat com front existente)
  const id =
  ...
  try {
    ...
    if (!parada.horaFinalizacao) {
      return res
        .status(409)
        .json({ ok: false, message: "Parada já está em aberto" });
    }
    ...
    console.error("[/api/paradas/desfazer] ERROR:", e);
    return res
      .status(500)
